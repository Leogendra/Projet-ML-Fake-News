...
def MyTestPipelines(models, X, y, score='accuracy'):
    ...
    for name, model in models:
        ...
        names.append(name)
        print("%s : %0.3f (%0.3f) in %0.3f s" %
              (name, cv_results.mean(), cv_results.std(), thetime))

    allresults = sorted(
        allresults, key=lambda result: result.scoremean, reverse=True)
